...
import toniqx.vailware.modules.misc.BlockReach;
...
public class PlayerControllerMP
{
    ...
    {
    	if(Client.getModule("ClickTP").isEnabled()) {
    		return 600;
    	}else {
	    	if(Client.getModule("BlockReach").isEnabled()) {
	    		return (float) BlockReach.reach.getValue();
	    	}else {
    ...
    {
        return (Client.getModule("Reach").isEnabled() && !this.currentGameType.isCreative() || this.currentGameType.isCreative());
    }
    ...
    public boolean onPlayerRightClick3d(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3d hitVec) {
    	...
    	if (!mc.theWorld.getWorldBorder().contains(hitPos)) {
    		...
    		netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(hitPos, side.getIndex(), null, f, f1, f2));
