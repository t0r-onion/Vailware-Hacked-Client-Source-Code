...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        ...
        mc.FontRendererNormal.drawString(this.field_148301_e.serverName, x + 32 + 3, y - 2, 16777215);
        List<String> list = this.mc.fontRendererObj.listFormattedStringToWidth(this.field_148301_e.serverMOTD, listWidth - 32 - 2);

        for (int i = 0; i < Math.min(list.size(), 2); ++i)
        {
        	mc.FontRendererArray.drawString((String)list.get(i), x + 32 + 3, y + 10 + 10 * i, 8421504);
        }

        String s2 = flag2 ? EnumChatFormatting.DARK_RED + this.field_148301_e.gameVersion : this.field_148301_e.populationInfo;
        int j = mc.FontRendererArray.getStringWidth(s2);
        mc.FontRendererArray.drawString(s2, x + listWidth - j - 15 - 18, y - 2, 8421504);
        int k = 0;
        String s = null;
        int l;
        String s1;
