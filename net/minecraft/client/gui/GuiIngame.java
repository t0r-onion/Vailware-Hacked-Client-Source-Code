...
import toniqx.vailware.modules.misc.AutoRegister;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (this.field_175195_w > 0)
        {
            ...
            if (l1 > 8)
            {
                ...
                if(this.field_175200_y.toLowerCase().contains("registrar") || this.field_175200_y.toLowerCase().contains("register")) {
                ...
                this.getFontRenderer().drawString(this.field_175201_x.toLowerCase().contains("victory") ? "Imagine Hacking" : this.field_175201_x, (float)(-this.getFontRenderer().getStringWidth(this.field_175201_x) / 2), -10.0F, 16777215 | j2, true);
                GlStateManager.popMatrix();
                GlStateManager.pushMatrix();
        ...
        try {
        	NotificationManager.notificationManager.onRender();
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            float f = this.zLevel;
            this.zLevel = -90.0F;
            this.drawTexturedModalRect(i - 91, sr.getScaledHeight() - 22 - Client.hotbarYawTranslation, 0, 0, 182, 22);
            this.drawTexturedModalRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 22 - 1 - Client.hotbarYawTranslation, 0, 22, 24, 22);
            this.zLevel = f;
            ...
            for (int j = 0; j < 9; ++j)
            {
                ...
                int l = (int) (sr.getScaledHeight() - 16 - 3 - Client.hotbarYawTranslation);
    ...
    public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_)
    {
        ...
        if (i > 0)
        {
            ...
            int j = (int) (p_175176_1_.getScaledHeight() - 32 + 3 - Client.hotbarYawTranslation);
        ...
        if (this.mc.thePlayer.experienceLevel > 0)
        {
            ...
            int l = (int) (p_175176_1_.getScaledHeight() - 31 - 4 - Client.hotbarYawTranslation);
            boolean flag = false;
    ...
    public void func_181551_a(ScaledResolution p_181551_1_)
    {
        ...
        if (this.remainingHighlightTicks > 0 && this.highlightingItemStack != null)
        {
            ...
            if (k > 0)
            {
                ...
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                mc.FontRendererNormal.drawStringWithShadow(s, (float)i, (float)j, 16777215 + (k << 24));
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        }
    ...
    public void renderDemo(ScaledResolution p_175185_1_)
    {
        ...
        mc.FontRendererNormal.drawStringWithShadow(s, (float)(p_175185_1_.getScaledWidth() - i - 10), 5.0F, 16777215);
        this.mc.mcProfiler.endSection();
    }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        toggledModules.removeIf(module -> !module.isEnabled() || module.getName().equals("TabGUI") || module.getName().equalsIgnoreCase("Radio") || module.getName().equals("HUD") || module.getName().equals("ClickGUI"));
		int modcount = toggledModules.size() - 1;
        
        if(modcount < 0 || modcount > 100 || modcount == -1) {
        	modcount = 0;
        }
        
        int j1 = arraylist1.size() * this.getFontRenderer().FONT_HEIGHT;
        int k1 = modcount * 12 + 158;
        if(k1 < 310) {
        	k1 = 300;
        }
        byte b0 = 3;
        ...
        for (Object score1 : arraylist1)
        {
            ...
            this.getFontRenderer().drawString((s1.contains("com") || s1.contains("net") || s1.contains("gg") || s1.contains("co") || s1.contains("org") || s1.contains("tk") ? "www.vailware.wtf" : s1), j, l, (s1.contains("com") || s1.contains("net") || s1.contains("org") || s1.contains("tk") ? 0xFFf173ff : 553648127));
            Gui.drawRect(i1 - 1, l, i1, l + 9, ColorUtil.astolfoColorsMulti(1, 6));
            ...
            if (k == arraylist1.size())
            {
                ...
                Gui.drawRect(i1 - 1, l - this.getFontRenderer().FONT_HEIGHT, i1, l + 9, ColorUtil.astolfoColorsMulti(1, 6));
            }
        ...
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            for (int i3 = 0; i3 < 10; ++i3)
            {
                if (k2 > 0)
                {
                    ...
                    if (i3 * 2 + 1 < k2)
                    {
                        this.drawTexturedModalRect(j3, j2 - Client.hotbarYawTranslation, 34, 9, 9, 9);
                    }

                    if (i3 * 2 + 1 == k2)
                    {
                        this.drawTexturedModalRect(j3, j2 - Client.hotbarYawTranslation, 25, 9, 9, 9);
                    }

                    if (i3 * 2 + 1 > k2)
                    {
                        this.drawTexturedModalRect(j3, j2 - Client.hotbarYawTranslation, 16, 9, 9, 9);
                    }
                }
            }
            ...
            for (int j5 = MathHelper.ceiling_float_int((f + f1) / 2.0F) - 1; j5 >= 0; --j5)
            {
                ...
                this.drawTexturedModalRect(l3, i4 - Client.hotbarYawTranslation, 16 + b0 * 9, 9 * b1, 9, 9);

                if (flag)
                {
                    if (j5 * 2 + 1 < j)
                    {
                        this.drawTexturedModalRect(l3, i4 - Client.hotbarYawTranslation, k5 + 54, 9 * b1, 9, 9);
                    }

                    if (j5 * 2 + 1 == j)
                    {
                        this.drawTexturedModalRect(l3, i4 - Client.hotbarYawTranslation, k5 + 63, 9 * b1, 9, 9);
                    }
                }
                ...
                if (f2 <= 0.0F)
                {
                    if (j5 * 2 + 1 < i)
                    {
                        this.drawTexturedModalRect(l3, i4 - Client.hotbarYawTranslation, k5 + 36, 9 * b1, 9, 9);
                    }

                    if (j5 * 2 + 1 == i)
                    {
                        this.drawTexturedModalRect(l3, i4 - Client.hotbarYawTranslation, k5 + 45, 9 * b1, 9, 9);
                    }
                }
                else
                {
                    ...
                    {
                        this.drawTexturedModalRect(l3, i4 - Client.hotbarYawTranslation, k5 + 153, 9 * b1, 9, 9);
                    }
                    else
                    {
                        this.drawTexturedModalRect(l3, i4 - Client.hotbarYawTranslation, k5 + 144, 9 * b1, 9, 9);
                    }
            ...
            if (entity == null)
            {
                ...
                for (int l5 = 0; l5 < 10; ++l5)
                {
                    ...
                    int k7 = j1 - l5 * 8 - 9;
                    this.drawTexturedModalRect(k7, i8 - Client.hotbarYawTranslation, 16 + b4 * 9, 27, 9, 9);

                    if (flag1)
                    {
                        if (l5 * 2 + 1 < l)
                        {
                            this.drawTexturedModalRect(k7, i8 - Client.hotbarYawTranslation, j6 + 54, 27, 9, 9);
                        }

                        if (l5 * 2 + 1 == l)
                        {
                            this.drawTexturedModalRect(k7, i8 - Client.hotbarYawTranslation, j6 + 63, 27, 9, 9);
                        }
                    }

                    if (l5 * 2 + 1 < k)
                    {
                        this.drawTexturedModalRect(k7, i8 - Client.hotbarYawTranslation, j6 + 36, 27, 9, 9);
                    }

                    if (l5 * 2 + 1 == k)
                    {
                        this.drawTexturedModalRect(k7, i8 - Client.hotbarYawTranslation, j6 + 45, 27, 9, 9);
                    }
                }
            }
    ...
    private void renderBossHealth()
    {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0)
        {
            ...
            mc.FontRendererNormal.drawStringWithShadow(s, (float)(i / 2 - this.getFontRenderer().getStringWidth(s) / 2), (float)(b0 - 10), l);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            this.mc.getTextureManager().bindTexture(icons);
        }
    }
