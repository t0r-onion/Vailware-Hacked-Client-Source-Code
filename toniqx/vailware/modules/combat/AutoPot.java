...
public class AutoPot extends Module {
    ...
    public static NumberSetting healthMin = new NumberSetting("Health", 12, 1, 20, 1);
    
    public AutoPot() {
        super("AutoPot", "", Keyboard.KEY_NONE, Category.COMBAT);
        this.addSettings(healthMin);
    }
    ...
    public void onEvent(Event e) {
    	try {
	        ...
	        if(e instanceof EventRotationMotion) {
	            if (!(Client.getModule("Fly").isEnabled()) && mc.thePlayer.onGround)
	                if (e.isPre()) {
	                    ...
	                    if (mc.thePlayer.getHealth() <= healthMin.getValue()) {
	                        if (timer.elapsed(50)) {
