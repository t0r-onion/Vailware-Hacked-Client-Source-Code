    ...
    public void renderPath() {
		...
		for (Vec3 loc: trailList) {
			...
			if (lastLoc == null) {
				...
				if (Minecraft.getMinecraft().thePlayer.getDistance(loc.getX(), loc.getY(), loc.getZ()) > 100) {
					
				}else {
					
					ScaffoldRenderUtils.drawLine(lastLoc.getX(), lastLoc.getY(), lastLoc.getZ(), loc.getX(), loc.getY(), loc.getZ());
